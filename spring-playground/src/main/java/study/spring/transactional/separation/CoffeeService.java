...
public class CoffeeService {
     ...
     * -> setAutoCommit을 false로 설정해준다.
     * (스프링부트 2버전부터는 auto-commit을 false로 설정하면 hibernate 설정이 자동으로 설정된다.)
     * 이 떄, getConnectionForTransasctionManagement() 를 수행하는 과정에서 DB connection의 획득을 나중으로 미룬다.
     *
     * @Override
     *        public void begin() {
     ...
     *    }
     *
     * 위 메소드를 실행할 때, short circuit으로 인해 앞쪽만 수행되고, 뒤쪽 메소드가 실행되지 않는다.
     * 즉, getConnectionForTransactionManagement()를 수행하지 않아서 물리적인 DB connection을 얻지 않는다.
     *
    ...
    public void makeCoffee(int amount) {
        ...
        System.out.println("actualTransactionActive = " + TransactionSynchronizationManager.isActualTransactionActive());
